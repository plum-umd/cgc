...
⟦⟧⸢↦ˢ⸣ {Γ} = witness-x (curry⸢λ↑⸣ id⸢ω↑⸣) $ mk[witness] fun ppr[fun]
  where
...
-- data acc[_] {𝓁} {A : Set 𝓁} (_R_ : relation 𝓁 A) (x : A) : Set 𝓁 where
--   Acc : (∀ y → y R x → acc[ _R_ ] y) → acc[ _R_ ] x
-- 
-- data widen-rel[_] {𝓁} {A : Set 𝓁} (_∇_ : A → A → A) : relation 𝓁 (A × A) where
...
-- pfp-widen : ∀ {𝓁} {A : Set 𝓁} {{A-PO : PreOrder 𝓁 A}} (_∇_ : A → A → A) (cmp : ∀ (x y : A) → x ⊴ y ⨄ not (x ⊴ y)) (f : A → A) (p : A × A) → acc[ widen-rel[ _∇_ ] ] p → ∃ x ⦂ A 𝑠𝑡 f x ⊴ x
