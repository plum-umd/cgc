...
data _↗♭_ {ℓ₁ ℓ₁ʳ ℓ₂ ℓ₂ʳ} (A : Set ℓ₁) {{_ : PreOrder ℓ₁ʳ A}} (B : Set ℓ₂) {{_ : PreOrder ℓ₂ʳ B}} : Set (ℓ₁ ⊔ᴸ ℓ₁ʳ ⊔ᴸ ℓ₂ ⊔ᴸ ℓ₂ʳ) where
  [λ_] : (f : A  → B) → {f-proper : proper (_≼_ ⇉ _≼_) f} → A ↗♭ B

module _ {ℓ₁ ℓ₁ʳ ℓ₂ ℓ₂ʳ} {A : Set ℓ₁} {{APO : PreOrder ℓ₁ʳ A}} {B : Set ℓ₂} {{BPO : PreOrder ℓ₂ʳ B}} where
  data _≼⸢↗♭⸣_ : relation (ℓ₁ ⊔ᴸ ℓ₁ʳ ⊔ᴸ ℓ₂ ⊔ᴸ ℓ₂ʳ) (A ↗♭ B) where
    λ⟨_⟩ :
  ...
  xRx⸢≼↗♭⸣ : reflexive _≼⸢↗♭⸣_
  xRx⸢≼↗♭⸣ {x = [λ f ] {f-proper}} = λ⟨ f-proper ⟩

  _⊚⸢≼↗♭⸣_ : transitive _≼⸢↗♭⸣_
  λ⟨ g≼h ⟩ ⊚⸢≼↗♭⸣ λ⟨ f≼g ⟩ = λ⟨ (λ x≼y → _⊚⸢≼⸣_ {{BPO}} (g≼h (xRx⸢≼⸣ {{APO}})) (f≼g x≼y)) ⟩
  ...
  instance
    ...
    PreOrder[↗♭] : PreOrder (ℓ₁ ⊔ᴸ ℓ₁ʳ ⊔ᴸ ℓ₂ ⊔ᴸ ℓ₂ʳ) (A ↗♭ B)
